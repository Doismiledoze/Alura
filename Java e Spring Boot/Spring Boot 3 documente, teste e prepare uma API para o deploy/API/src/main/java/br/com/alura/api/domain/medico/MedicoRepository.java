...
public interface MedicoRepository extends JpaRepository<Medico, Long> {
    ...
    @Query("SELECT m FROM Medico m WHERE m.ativo=true AND m.especialidade= :especialidade AND m.id NOT IN (SELECT c.medico.id FROM Consulta c WHERE c.data = :data AND c.motivoCancelamento IS null) ORDER BY random() LIMIT 1")
    Medico escolherMedicoAleatorioLivreNaData(Especialidade especialidade, LocalDateTime data);
